...
if area <= 66200:python coloridentification.py 
...
while (1):
    ...
    if clicked:
        recEnd = (round(width * .97 ), round(height * .1))
        ...
        if r + g + b >=600:
            cv2.putText(img, text, textStart, cv2.FONT_HERSHEY_DUPLEX, 0.8, (0, 0, 0), 1, cv2.LINE_AA)
        else:
            cv2.putText(img, text, textStart, cv2.FONT_HERSHEY_DUPLEX, 0.8, (255, 255, 255), 1, cv2.LINE_AA)
        clicked = False
